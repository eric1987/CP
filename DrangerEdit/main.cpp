...
int packet_queue_put(PacketQueue *q, AVPacket *pkt) {
	...
	AVPacketList *pkt1;
	if (pkt != &flush_pkt && av_packet_ref(pkt, nullptr) < 0) {
		return -1;
	}
	...
	lock_guard<mutex> lockGuard(q->mutex);
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block)
{
	...
	//SDL_LockMutex(q->mutex);
	unique_lock<mutex> mlock(q->mutex);
...
#endif
		lock_guard<mutex> lockGuard(screen_mutex);
		SDL_UpdateTexture(texture, &rect, vp->disFrame->data[0], vp->disFrame->linesize[0]);
...
#endif
			lock_guard<mutex> lockGuard(is->pictq_mutex);
			is->pictq_size--;
			is->condVar.notify_one();
		}
	}
	else {
...
int queue_picture(VideoState *is, AVFrame *pFrame, double pts) {
	...
	VideoPicture *vp;
	int dst_pix_fmt;
	//AVPicture pict;	//deprecated. use AVFrame instead.

	/* wait until we have space for a new pic */
	//SDL_LockMutex(is->pictq_mutex);
	unique_lock<mutex> lockP(is->pictq_mutex);

	while (is->pictq_size >= VIDEO_PICTURE_QUEUE_SIZE &&
		!is->quit) {
		//SDL_CondWait(is->pictq_cond, is->pictq_mutex);
		is->condVar.wait(lockP);
	}
	//SDL_UnlockMutex(is->pictq_mutex);
	...
	vp->disFrame = av_frame_alloc();
	/* We have a place to put our picture on the queue */
	sws_scale(is->sws_ctx, (uint8_t const * const *)pFrame->data,
		pFrame->linesize, 0, is->video_ctx->height,
		vp->disFrame->data, vp->disFrame->linesize);
...
int audio_thread(void *arg)
{
	...
	QIODevice *out = is->audio->start();
	uchar *med_buffer = (uint8_t*)malloc(MAX_AUDIO_FRAME_SIZE + FF_INPUT_BUFFER_PADDING_SIZE);

	uint8_t **buffer;
	SwrContext *swr_ctx = swr_alloc();
	swr_ctx = swr_alloc_set_opts(swr_ctx, is->audio_ctx->channel_layout,
		AV_SAMPLE_FMT_S16,
	...
	for (;;) {
		...
		avcodec_send_packet(is->audio_ctx, packet);
		...
		while (packet->size > 0)
		{
			int len, data_size = MAX_AUDIO_FRAME_SIZE;
				...
				ret = avcodec_receive_frame(is->audio_ctx, pFrame);
				...
				int outsize = av_samples_get_buffer_size(NULL, is->audio_ctx->channels,
					pFrame->nb_samples,
					AV_SAMPLE_FMT_S16,
					0);
		...
		//pts = synchronize_audio(is, pFrame, pts);
...
int stream_component_open(VideoState *is, int stream_index) {
	...
	codecCtx->pkt_timebase = pFormatCtx->streams[stream_index]->time_base;
...
#if 0	//change from SDL to QAudioOutput
...
#endif
		is->audio = new QAudioOutput(format);
	...
	case AVMEDIA_TYPE_AUDIO:
		...
		is->audio_th = new thread(audio_thread, ref(is));
		//SDL_PauseAudio(0);
		break;
	case AVMEDIA_TYPE_VIDEO:
		...
		is->video_th = new thread(video_thread, ref(is));
...
int decode_thread(void *arg) {
	...
	for (;;) {
		...
		if (is->seek_req) {
			...
			baseq.num = 1;
			if (stream_index >= 0) {
				seek_target = av_rescale_q(seek_target, baseq,
...
int main3(int argc, char *argv[]) {
	...
	screen = SDL_CreateWindow("title", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 640, 480, SDL_WINDOW_OPENGL);
	...
	char *url = "D:/work/Resource/oceans_264.h264";
	...
	is->parse_th = new thread(decode_thread, ref(is));
	...
	for (;;) {
		...
		case SDL_QUIT:
			...
			*/
			...
			is->audioq.condVar.notify_one();
			is->videoq.condVar.notify_one();
			SDL_Quit();
			return 0;
			break;
...
void devpkt(queue<AVPacket *> &q, AVPacket *pkt)
{
...
void deapkt(queue<AVPacket *> &q, AVPacket *pkt)
{
...
void deframe(queue<AVFrame *> &q, AVFrame *frame)
{
...
int main(int argc, char *argv[])
{
	...
	if (audio_index >= 0)
	{
		...
		audio_fmt.setSampleRate(48000);	//change by specific condition
	...
	unsigned char *out_buffer = (unsigned char *)av_malloc(
		av_image_get_buffer_size(AV_PIX_FMT_RGB32, vctx->width, vctx->height, 1));
	av_image_fill_arrays(disframe->data, disframe->linesize, out_buffer,
		AV_PIX_FMT_RGB32, vctx->width, vctx->height, 1);
	
	SwsContext *s_ctx = nullptr;
	s_ctx = sws_getContext(vctx->width, vctx->height, vctx->pix_fmt,
		vctx->width, vctx->height, AV_PIX_FMT_RGB32, SWS_BICUBIC,
		nullptr, nullptr, nullptr);
	...
	while (true)
	{
		...
		sws_scale(s_ctx, (uint8_t const* const*)frame->data, frame->linesize,
			0, vctx->height, disframe->data, disframe->linesize);
